...
class ResponseBuilder:
    ...
    def build(self) -> bytes:
        ...
        """
        # TODO: this function
        response_str = self.status + NEWLINE
        for header in self.headers:
            response_str += header
            response_str += NEWLINE
        response_str += NEWLINE
        response_str = response_str.encode('utf-8')
        if self.content:
            response_str += self.content
        return response_str
...
class HTTPServer:
    ...
    def private_req(self, request):
        authenticated = False
        response = ResponseBuilder()
        for k, v in request.headers.items():
            if k == 'authorization':
                decoded = b64decode(v.replace(' Basic', '')).decode()
                usernameAndPassword = decoded.split(':')
                if usernameAndPassword[0] == 'admin' and usernameAndPassword[1] == 'password':
                    authenticated = True
                else:
                    content = get_file_contents('401.html')
                    response.set_status("401", "Unauthorized")
                    response.add_header("Content-Length", len(content.encode('utf-8')))
                    response.set_content(content)
                    return response
        if authenticated:
            content = get_file_contents(request.path)
            response.add_header("Content-Length", len(bytes(content, encoding="utf-8")))
    ...
    def redirect_req(self, request):
        query = request.path.replace('redirect?query_string=', '')
        response = ResponseBuilder()
        response.set_status(307, "Temporary Redirect")
        response.add_header("Location", "https://youtube.com/results?search_query=" + query)
        return response
    ...
    def get_request(self, request: Request) -> Response:
        ...
        if request.path == 'favicon.ico':
        ...
        elif request.path == 'private.html':
            response = self.private_req(request)
        elif 'redirect' in request.path:
            response = self.redirect_req(request)
        elif not file_exists(request.path):
            return self.resource_not_found()
        else:
            extension = path.splitext(request.path)[1]
            if should_return_binary(extension):
                content = get_file_binary_contents(request.path)
            ...
            response.set_status(200, "OK")
            response.add_header("Content-Type", get_file_mime_type(extension))
            response.set_content(content)
    ...
    def post_request(self, request: Request) -> Response:
        ...
        split_content = request.content.split('&')

        key_val_rows = []
        for contents in split_content:
            key_val_rows.append(contents.split('='))

        for row in key_val_rows:
            for index in range(len(row)):
                row[index] = unquote_plus(row[index])
        ...
        for row in key_val_rows:
            html_response += '<tr>'
            for index in range(len(row)):
                html_response += f'<td>{row[index]}</td>'
            html_response += '</tr>'
        html_response += "</table></body></html>"
        
        response = ResponseBuilder()
        content = html_response.encode(encoding='utf-8')
        response.set_status(200, "OK")
        response.add_header("Content-Type", "text/html")
        response.add_header("Content-Size", len(content))
        response.set_content(content)
    ...
    def resource_not_found(self) -> Response:
        ...
        builder.add_header("Content-Length", len(content.encode('utf-8')))
